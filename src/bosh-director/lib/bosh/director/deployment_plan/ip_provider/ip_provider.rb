    ...
    class IpProvider
      ...
      def reserve_existing_ips(reservation)
        ...
        when DynamicNetwork
          ...
          #
          # TODO: can we get rid of network_type? it should already be dynamic (BL, MR)
      ...
      def reserve_manual_with_subnet(reservation, subnet)
        @logger.debug("Marking existing IP #{format_ip(reservation.ip)} as reserved for manual network #{reservation.network.name}")
